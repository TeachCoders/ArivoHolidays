...
export default async function TourPackageBySlugPage({ params }: Props) {
  const { slug } = await params;
  const journey = await fetchBySlug<Journey>("/journey/by-slug", slug);
  const countrySlug = journey?.cities?.[0]?.state?.country?.slug;
  ...
  const schema = journey
    ? [
        ...
        breadcrumbSchema([
          { name: "Home", path: "/" },
          { name: countryTitle || "Tour Packages", path: countrySlug ? `/${countrySlug}/tour-packages` : "/tour-packages" },
