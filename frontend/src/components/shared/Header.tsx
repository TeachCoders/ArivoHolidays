...
const DEFAULT_DESTINATION_LINKS: NavChild[] = [
  ...
  { href: "/india/rajasthan", label: "Rajasthan" },
  { href: "/india/uttar-pradesh", label: "Uttar Pradesh" },
  { href: "/india/uttarakhand", label: "Uttarakhand" },
  { href: "/india/himachal-pradesh", label: "Himachal Pradesh" },
  { href: "/india/jammu-and-kashmir", label: "Jammu and Kashmir" },
  { href: "/india/punjab", label: "Punjab" },
  { href: "/india/ladakh", label: "Ladakh" },
  { href: "/india/delhi", label: "Delhi NCR" },
];
  ...
  const destinationTree = useMemo(() => {
    ...
    states.forEach(state => {
      ...
      countryEntry.states.push({
        ...
        slug: state.slug,
        href: `/${state.country.slug}/${state.slug}`,
        displayOrder: state.displayOrder && state.displayOrder > 0 ? state.displayOrder : 999,
        cities: sortedCities
      });
    });
    
    const result = Array.from(countryMap.values()).map(country => {
    ...
    () =>
      pickPriorityLinks<Journey>(
        ...
        (j) => {
          ...
          return {
            href: countrySlug
              ? `/${countrySlug}/tour-packages/${j.slug}`
              : `/tour-packages/${j.slug}`,
  ...
  const navLinks: NavLink[] = useMemo(
    () => [
      ...
      },
      {
        ...
        dropdownStyle: "mega",
        dropdownColumns: 2,
        seeAllHref: "/india/tour-packages",
        ...
        <div className="flex items-stretch justify-between gap-4">
          <Link href="/" className="flex items-center gap-2 group shrink-0">
            <img
              ...
              alt="Arivo Holiday"
              className="h-[88px] w-auto object-contain transition-transform duration-300 group-hover:scale-[1.03]"
            />
          </Link>
              ...
              >
                {link.children || link.isDestinationMega ? (
                  <button
                    ...
                    className={`relative px-3 py-2.5 text-sm font-medium tracking-wide rounded-full transition-all duration-300 group flex items-center gap-1 hover:text-[#2E8B8B] hover:bg-[#2E8B8B]/5 cursor-pointer ${
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-3.5 h-3.5 relative z-10 transition-transform duration-300 ${
                        openDesktopDropdown === link.href ? "rotate-180" : ""
                      }`}
                    />
                  </button>
                ) : (
                  <Link
                    href={link.href}
                    className={`relative px-3 py-2.5 text-sm font-medium tracking-wide rounded-full transition-all duration-300 group flex items-center gap-1 hover:text-[#2E8B8B] hover:bg-[#2E8B8B]/5 ${
                      openDesktopDropdown === link.href
                        ? "text-[#2E8B8B] bg-[#2E8B8B]/5"
                        : "text-[#666]"
                    }`}
                ...
                {link.children && link.dropdownStyle === "mega" && !link.isDestinationMega && (
                  <div
                    ...
                    className={`absolute left-1/2 -translate-x-1/2 top-full pt-2.5 transition-all duration-200 ${
                      openDesktopDropdown === link.href
                  ...
                  >
                    <div
                      className={`bg-white rounded-none border border-[#ececec] shadow-[0_16px_40px_-8px_rgba(0,0,0,0.14)] p-5 ${
                        link.dropdownColumns === 1 ? "w-[320px]" : "w-[680px]"
                      }`}
                    >
                      <span className="inline-block text-[19px] font-bold text-[#2E8B8B] mb-2">
                        {link.dropdownTitle ?? "Popular in India"}
                      </span>
                      <div
                        className={`grid gap-x-6 gap-y-1 mt-1 ${
                          link.dropdownColumns === 1
                            ? "grid-cols-1"
                      ...
                      >
                        {link.children.map((child) => (
                          <Link
                            ...
                            className="py-0.5 text-[17px] text-[#555] hover:text-[#2E8B8B] transition-colors"
                          >
                            {child.label}
                          </Link>
                        ))}
                      ...
                      {link.seeAllHref && (
                        <div className="mt-3 pt-3 border-t border-[#ececec] flex items-center justify-between">
                          ...
                          <Link
                            ...
                            onClick={() => setOpenDesktopDropdown(null)}
                            className="text-[16px] font-bold text-[#D4561A] hover:opacity-75 transition-opacity flex items-center gap-1"
                          >
                            See All
          ...
          <div className="hidden lg:flex items-center gap-3">
            <QuoteModal>
              <button className="btn-primary px-5 py-2.5 text-sm font-medium tracking-wide flex items-center gap-2 shadow-[0_1px_2px_rgba(0,0,0,0.06)] hover:shadow-[0_4px_14px_-4px_rgba(0,0,0,0.18)] transition-shadow duration-300">
