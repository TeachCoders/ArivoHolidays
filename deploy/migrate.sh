...
echo "  bash /opt/arivo/app/deploy/backup.sh"
echo "  # backup file: /opt/arivo/app/Backend/backups/db_*.dump + uploads_*.tar.gz"
echo "  # private documents (documents/, user/) ho to:"
echo "  tar -czf /tmp/private_$(date +%F).tar.gz -C /opt/arivo/app/Backend/public documents user"
...
echo "  nano /opt/arivo/app/Backend/.env       # DATABASE_URL wahi Supabase ka, SESSION_SECRET naya random"
echo "  nano /opt/arivo/app/frontend/.env      # wahi public URLs"
echo "  bash /opt/arivo/app/deploy/deploy-app.sh"

log "Extra: private documents ==>
echo "  ssh root@<NEW_IP>"
echo "  cd /opt/arivo/app/Backend && mkdir -p public && tar -xzf /tmp/private_*.tar.gz -C public"
