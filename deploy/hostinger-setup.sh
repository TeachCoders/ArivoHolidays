...
echo "║     nano /opt/arivo/app/Backend/.env                         ║"
echo "║     nano /opt/arivo/app/frontend/.env                        ║"
echo "║                                                              ║"
echo "║  2. Deploy karo:                                             ║"
echo "║     bash /opt/arivo/app/deploy/deploy-app.sh                 ║"
echo "║                                                              ║"
echo "║  3. DNS setup karo (Hostinger hPanel → DNS):                 ║"
echo "║     A  arivoholidays.com  → <VPS_IP>                         ║"
echo "║     A  api.arivoholidays.com → <VPS_IP>                      ║"
echo "║     A  www               → <VPS_IP>                         ║"
echo "║                                                              ║"
echo "║  4. SSL setup (DNS point hone ke baad):                      ║"
echo "║     certbot --nginx -d arivoholidays.com -d www.arivoholidays.com -d api.arivoholidays.com --redirect --agree-tos -m aap@email.com"
echo "║                                                              ║"
echo "║  5. Backup cron add karo:                                    ║"
echo "║     crontab -e → 0 3 * * * bash /opt/arivo/app/deploy/backup.sh >> /var/log/arivo-backup.log 2>&1"
echo "╚══════════════════════════════════════════════════════════════╝"
