...
async function updateStates() {
  console.log(`Starting pageOverview update for all ${states.length} states...`);
  let successCount = 0;
  ...
  for (const state of states) {
    try {
      const result = await prisma.state.updateMany({
        ...
        data: { overView: state.html }
      });
      if (result.count > 0) {
        console.log(`✅ Updated: ${state.slug}`);
        successCount++;
      } else {
        console.log(`⚠️ Not found/No update: ${state.slug}`);
      }
    } catch (e) {
      console.error(`❌ Error updating ${state.slug}:`, e.message);
    }
  }
  console.log(`\nUpdate completed! Successfully updated ${successCount} states.`);
}

updateStates().finally(() => prisma.$disconnect());
